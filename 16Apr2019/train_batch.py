...
if which_computer==4:
    ...
    root_path = '/Users/raghuram.b/Desktop/Predictive-Maintenance-master/dataset/raghu/'
else:
    root_path = ''
...
#sample_rates_n_seq = {'sample_1_day':(1,2)}

for name in sensor_names:
    print("sensor name",name)
    for j in sample_rates_n_seq:
        print("sample_rates_n_seq(category of time):",j)
        n_seqs = sample_rates_n_seq[j]
        print("n_sequences numbers(step of time)",n_seqs)
        sample_rate = j
        print("sample_rate:",j)
        for s in n_seqs:
            print("S value:",s)
            print(s,n_epochs,dataset_path,name,sample_rate,train,root_path,save_info)
