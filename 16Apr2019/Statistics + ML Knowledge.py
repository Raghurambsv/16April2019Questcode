...
NOTE:  linear regression is a regression algorithm whereas logistic regression is a classification algorithm.
       Model Equalation metrics ==>REGRESSION problems uses RootMeanSquaredError
                                   CLASSIFICATION problems use ACCRUACY,confusion matrix
...
FEATURE SELECTION METHODS (All present in IOT_AIRCRAFT_DATA.py...savitha shared code from hyd female)
=========================
 1)Random forest can help in choosing the best & imp features from the dataset
 2)There is Borutapy method 
 
 3) **** Select K Best Feautures based on chi-square statistical tests *********************
 ...
 4) Features Selected according to PCA which explain the maximum variance
 
 5) Using Recursive Feature Elimination (with logistic Regression)
...
confusion matrix
================
. It is used only for Classification models (It mostly used for binary models....in other words if multiple output class than its difficult to interpret the confusion matrix)
. TN and TP is along the diagonal (resembling the backward slash).....so if numbers are huge along this diagonal than the model is good
. in scikit learn....rowwise is actual values...column wise is predicted values 

        Predicted
Actual [ TN  FP]
Actual [ FN  TP]

. Accruacy = (TN + TP)/ Total   
. ErrorRate = 1 - Accruacy  
. Recall = TP/(TP + FN) (Hint: TP/second_row)
. Precision= TP/TP+FP (Hint: TP/second_column)
...
The Reciever Operating Characteristic (ROC) curve graphs the true positive rate versus the false positive rate:
 ...
 Approach for any ML problems   (to feature selection ==> Airbus code or HomeCredit.py
=============================                                  &
                                                          https://www.kaggle.com/willkoehrsen/start-here-a-gentle-introduction)

. find the types of all columns
. convert all the object types to numbers
. find missing values in all columns and delete them or fill them with appropriate values (mean or std)
. LightGBM algorithm can deal with categorial column with string directly
. if the categories in the column is <= 2 (Male/Female) use labelencoder (no extra columns it will create ....same column converts into arbitary numbers..if u want to give weightage then these arbitary numbers will mislead)
. if categorical column has more than 2 categories...go for one hot encoding or dummies...after doing dummies for train & test dataset ...you need to align train & test...coz some cols might be missing in test dataset ..so need to ALIGN Train & Test
. if more features are there than you can do getdummies followed by PCA to reduce the dimensions
. list of features having low or no correlation with regression label will be removed from feature list
. list of features with very low or no variance found using frequency distribution by standard deviation will be removed
. Polynomial Features (combine 2 features in various ways and see the coorelation with target)
. plot graphs to see the pattern....plot boxplot to see how many outliers are there...where is all the data mostly grouped in what quartile
. features selected after feature selection + extra derived features both combine to form the input for the alogs
.KERNEL DENSITY PLOT ...its a plot to find the graph between a highly correlated variable to target value ( if the target is binary you get 2 lines for each target value...if target is multi class than you will have taht many lines in the graph)

.  classification problems==> Accruacy of algos ...confusion matrix....f1 score(recall & precision)
                              from sklearn import metrics
                              metrics.confusion_matrix(y_test, y_pred)
                              metrics.classification_report(y_test, y_pred)
                              metrics.accuracy_score(y_test, y_pred)
                              
.  Regression problems ==>  'Root Mean Squared Error' : metrics.mean_squared_error(actual, predicted)**0.5,
...
word2vec
========
https://nlpforhackers.io/word-embeddings/ -plotting graph for word2vec
https://www.kaggle.com/sumitdua10/cnn-notebook-on-text-classification  -CNN Notebook on Text Classification
https://www.kaggle.com/xchmiao/popcorn-rnn-model - popcorn RNN model
https://www.kaggle.com/jungealexander/word2vec-and-random-forest-classification -Randomforest
https://www.kaggle.com/arunava21/word2vec-and-random-forest-classification -RandomForest
https://github.com/nadbordrozd/blog_stuff/blob/master/classification_w2v/benchmarking_python3.ipynb --How to convert word2vec to array


https://medium.com/all-things-ai/in-depth-parameter-tuning-for-svc-758215394769 - FOR ALL ALGORITHMS VISUAL EXPLANATION
https://towardsdatascience.com/journey-to-the-center-of-multi-label-classification-384c40229bff-Multilabel
https://www.analyticsvidhya.com/blog/2017/08/introduction-to-multi-label-classification/-Multilabel

Apache Tika
===========
https://wiki.apache.org/tika/ComparisonTikaAndPDFToText201811
http://www.mathdoc.fr/publis/d7.2-v1.0.pdf


PDF working links
=================
https://reachtim.com/articles/PDF-Manipulation.html
https://gist.github.com/roliveira/a18f6a16754edc9caa3424d9fa1e5d6d
...
https://github.com/jsvine/pdfplumber/blob/master/examples/notebooks/san-jose-pd-firearm-report.ipynb
https://stackoverflow.com/questions/35344982/itext-extracted-text-from-pdf-file-using-locationtextextractionstrategy-is-in-w
https://pdfcrowd.com/doc/api/image-to-pdf/python/
https://github.com/jsvine/pdfplumber